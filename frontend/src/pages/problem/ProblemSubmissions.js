        ...
        <DropdownFrame title={t("problem_submissions.filter")}>
            <div className="px-8 py-6 flex flex-col">
                {isLoggedIn && <div className="mb-3">
                    <Checkbox label={t("problem_submissions.own_solutions")} onChange={setOnlyOwn}
                              initChecked={onlyOwn}/>
                </div>}
                <div className="mb-5">
                    <Checkbox label={t("problem_submissions.full_solutions")} onChange={setOnlyAC}
                              initChecked={onlyAC}/>
                </div>
                <div className="flex justify-center">
                    <button className="mr-1 btn-indigo padding-btn-default w-32"
                            onClick={handleSubmit}>{t("problem_filter.search")}</button>
                    <button className="ml-1 btn-gray padding-btn-default w-32"
                            onClick={handleReset}>{t("problem_filter.reset")}</button>
                </div>
            </div>
        </DropdownFrame>
    )
