...
function TestCase({ index, numCases, testCase, group, isLastGroup, isLastCase }) {
    ...
    return (
        <tr>
            {index === 0 && <>
                <td className={`padding-td-default border border-t-0 border-divide-col text-center ${bottomBorderGroup}`} rowSpan={numCases}>
                    <div className="flex flex-col justify-center">
                        <div className="flex items-center justify-center mb-2">
                            {group["failed"] && <SVGWrongSimple cls="w-7 h-7 text-red-500" />}
                            {!group["failed"] && group["completed"] && <SVGCorrectSimple cls="w-7 h-7 text-indigo-500" />}
...
function TestGroup({ group, isLast }) {
    ...
    const testCaseContents = testCases.map((testCase, index) =>
        <TestCase index={index} numCases={testCases.length} testCase={testCase} group={group} key={index} isLastGroup={isLast} isLastCase={index === testCases.length - 1} />
    )
    return (
        <>
            {testCaseContents}
