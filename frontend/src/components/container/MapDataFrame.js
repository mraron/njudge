...
function MapDataFrame({data, title, titleComponent}) {
    const rows = data.map((pair, index) =>
        <tr className="divide-x divide-default" key={index}>
            <td className="padding-td-default bg-grey-800 font-medium align-top whitespace-nowrap" style={{maxWidth: 0}}>{pair[0]}</td>
            <td className="padding-td-default bg-grey-825 break-words w-0">
                {pair[1]}
            </td>
        </tr>
    );
