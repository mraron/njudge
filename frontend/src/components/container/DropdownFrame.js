    ...
    return (
        <RoundedFrame>
            <button
                ...
                className={`w-full px-8 py-2.5 ${
                    isOpen
                        ...
                        : "bg-frame-bg hover:bg-grey-775 rounded-container"
                } border-border-def flex items-center justify-center`}>
            ...
            <div className={`${isOpen ? "" : "h-0 overflow-hidden"}`}>
                <div className="border-t border-border-def">{children}</div>
            </div>
        </RoundedFrame>
    )
}
