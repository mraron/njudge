...
func (problemCategoryL) LoadParentProblemCategories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProblemCategory interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`problem_categories`),
		qm.WhereIn(`problem_categories.parent_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (problemCategoryL) LoadCategoryProblemRels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProblemCategory interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`problem_rels.category_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ProblemCategory) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProblemCategory) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(problemCategoryAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(problemCategoryPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert problem_categories, could not build conflict column list")
			}

			conflict = make([]string, len(problemCategoryPrimaryKeyColumns))
			copy(conflict, problemCategoryPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"problem_categories\"", updateOnConflict, ret, update, conflict, insert, opts...)
