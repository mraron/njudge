...
func (problemRelL) LoadCategory(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProblemRel interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`problem_categories.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (problemRelL) LoadProblemProblemTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProblemRel interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`problem_tags.problem_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (problemRelL) LoadProblemSubmissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProblemRel interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`submissions`),
		qm.WhereIn(`submissions.problem_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ProblemRel) AddProblemProblemTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ProblemTag) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.ProblemID, rel.TagID}
...
func (o *ProblemRel) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProblemRel) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(problemRelAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(problemRelPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert problem_rels, could not build conflict column list")
			}

			conflict = make([]string, len(problemRelPrimaryKeyColumns))
			copy(conflict, problemRelPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"problem_rels\"", updateOnConflict, ret, update, conflict, insert, opts...)
