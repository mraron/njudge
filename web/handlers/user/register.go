...
func Register(cfg config.Server, DB *sqlx.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		transaction := func() {
			...
			m.Message = fmt.Sprintf(`Kedves %s!<br> Köszönjük regisztrációd. Aktiváló link: <a href="http://`+cfg.Hostname+`/user/activate/%s/%s">http://`+cfg.Hostname+`/user/activate/%s/%s</a>`, c.FormValue("name"), c.FormValue("name"), key, c.FormValue("name"), key)
