...
var (
	judgeDBTypes = map[string]string{`Host`: `text`, `ID`: `integer`, `Online`: `boolean`, `Ping`: `integer`, `Port`: `text`, `State`: `text`}
	_            = bytes.MinRead
)
...
func testJudgesUpdate(t *testing.T) {
	...
	if len(judgeColumns) == len(judgePrimaryKeyColumns) {
...
func testJudgesSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(judgeColumns, judgePrimaryKeyColumns) {
		fields = judgeColumns
	} else {
