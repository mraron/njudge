...
var (
	...
	problemCategoryColumnsWithoutDefault = []string{"name", "parent_id"}
	problemCategoryColumnsWithDefault    = []string{"id"}
	problemCategoryPrimaryKeyColumns     = []string{"id"}
)

type (
	// ProblemCategorySlice is an alias for a slice of pointers to ProblemCategory.
	// This should generally be used opposed to []ProblemCategory.
...
// CountG returns the count of all ProblemCategory records in the query, and panics on error.
...
func (o *ProblemCategory) ParentProblemCategories(mods ...qm.QueryMod) problemCategoryQuery {
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"problem_categories\".*"})
	}
...
func (o *ProblemCategory) CategoryProblemRels(mods ...qm.QueryMod) problemRelQuery {
	...
	query := ProblemRels(queryMods...)
	queries.SetFrom(query.Query, "\"problem_rels\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"problem_rels\".*"})
	}
...
func (problemCategoryL) LoadParent(e boil.Executor, singular bool, maybeProblemCategory interface{}, mods queries.Applicator) error {
	...
	if singular {
		object = maybeProblemCategory.(*ProblemCategory)
	} else {
		slice = *maybeProblemCategory.(*[]*ProblemCategory)
	}
...
func ProblemCategories(mods ...qm.QueryMod) problemCategoryQuery {
	...
	return problemCategoryQuery{NewQuery(mods...)}
}
